...
class MultWaveGCUNet(nn.Module):
    """
        Using Multi-scale DWT decomposition,
        different frequency components are input into GCN as features to aggregate node features,
        and finally IDWT is used to restore the aggregated features layer by layer.
        During the restoration process, the losses are calculated separately, which will be used in the final training.
    """
    def __init__(self, input_channel, embedding_dim=64, top_k=30, input_node_dim=2, graph_alpha=3, device=torch.device('cuda:1'), gc_depth=1, batch_size=128):
        super(MultWaveGCUNet, self).__init__()

        #Multilevel DWT
        self.wave_decompose = WaveDecompose(input_channel)

        # Dynamic graph network
        self.graph1 = EmbeddingGraph(input_channel, embedding_dim, top_k, graph_alpha, device)
        self.graph2 = EmbeddingGraph(input_channel, embedding_dim, top_k, graph_alpha, device)
        self.graph3 = EmbeddingGraph(input_channel, embedding_dim, top_k, graph_alpha, device)
    ...
    def forward(self, x, idx, device):
        ...
        # latent_representation1：wave_latent3
