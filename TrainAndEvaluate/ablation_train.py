...
import numpy as np
import os
from .ablation_evatuate import evaluate_loss
    ...
    for epoch in range(args.epochs):
        ...
        for i, x in enumerate(tqdm(train_loader, desc='EPOCH: [%d/%d]' % (epoch + 1, args.epochs))):
            ...
            if args.use_dis:
                ...
                # print("detil loss ::::{0},{1},{2},{3},{4}".format(criterion_rec(output[0][0], output[0][1]), criterion_rec(output[1][0], output[1][1]), criterion_rec(output[2][0], output[2][1]),
                #                                    criterion_rec(output[3][0], output[3][1]), criterion_rec(output[4][0], output[4][1])))
                ...
                #            +args.d*criterion_rec(output[3][0], output[3][1])
                rec_loss = criterion_rec(output, x)
                # rec_loss = args.a * criterion_rec(output[0][0], output[0][1]) + args.b * criterion_rec(output[1][0], output[1][1]) + args.c * criterion_rec(output[2][0], output[2][1])
                # rec_loss = args.a*criterion_rec(output[0][0], output[0][1])+args.b*criterion_rec(output[1][0], output[1][1])+args.c*criterion_rec(output[2][0], output[2][1])\
                #            +args.d*criterion_rec(output[3][0], output[3][1])

                # rec_loss = args.e*criterion_rec(output[4][0], output[4][1])
