...
class MultWaveGCUNet1DecomposeLayerWithGCN(nn.Module):
    """
        Only use one Decompose Layer with GCN
    """
    def __init__(self, input_channel, embedding_dim=64, top_k=30, input_node_dim=2, graph_alpha=3, device=torch.device('cuda:1'), gc_depth=1, batch_size=128):
        super(MultWaveGCUNet1DecomposeLayerWithGCN, self).__init__()
    ...
    def forward(self, x, idx, device):
        ...
        # reconstruction loss0：x
        # reconstruction loss1：wave_high1
