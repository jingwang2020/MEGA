...
from AstMad.stgcn import sgcn, en_tcn, de_tcn
from .wavemodule import Dwtconv, Transition, Bottleneck, SingleLayer, DWTCompose, IDWT, DWTCompose1Layer
...
class NodeGraph(nn.Module):
    ...
    def forward(self, low_frequency, high_frequency):
        ...
        # vec1 = torch.tanh(self.alpha*self.linear1(vec1))
        # vec2 = torch.tanh(self.alpha*self.linear2(vec2))
...
class EmbeddingGraph(nn.Module):
    def __init__(self, nodes, dim, top_k, alpha, device):
        ...
        self.alpha = alpha
        self.device = device
        self.layer_norm = nn.LayerNorm(nodes)
        #
        self.LeReLu = nn.LeakyReLU()
    ...
    def forward(self, index_input):
        ...
        # matrix = self.layer_norm(matrix)
        # adj = torch.sigmoid(matrix)*self.LeReLu(matrix)
...
class STGCNEncoder(nn.Module):
    def __init__(self, st_layers=2, input_node_channel=16, conv_node_channel=16, end_node_channel=32, output_node_channel=64, gcn_depth=2, hopalpha=0.05, dropout=0.2):
        ...
        for i in range(st_layers):
            ...
            # self.tc.append(nn.Conv2d(input_node_channel, conv_node_channel, (1, 4)))
            # self.tc.append(nn.BatchNorm2d(conv_node_channel))
            ...
            # self.norm.append(nn.BatchNorm2d(input_node_channel))
...
class WaveDecompose1Layer(nn.Module):
    def __init__(self, input_channel):
        super(WaveDecompose1Layer, self).__init__()
        self.dwt = DWTCompose1Layer(input_channel)
...
class WaveGCN(nn.Module):
    ...
    def __init__(self, gcn_depth=2, hopalpha=0.05):
        ...
        # self.gcn1 = sgcn(conv_node_channel, input_node_channel, gcn_depth, hopalpha)
        # self.gcn2 = sgcn(conv_node_channel, input_node_channel, gcn_depth, hopalpha)
        # self.gcn3 = sgcn(conv_node_channel, input_node_channel, gcn_depth, hopalpha)
